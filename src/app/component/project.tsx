  ...
  return (
    <div className="flex flex-col h-full select-none w-auto overflow-hidden">
      <div className="flex justify-between h-[50px] border-b border-neutral-950">
        <div className="flex min-w-[512px] max-w-[1024px] h-[34px] m-[8px] ml-4 rounded overflow-x-auto overflow-y-hidden">
          <FilterTagView handleRemoveTagFromFilter={props.handleRemoveTagFromFilter} tags={props.searchTags} color="blue" join="and" />
          <FilterTagView handleRemoveTagFromFilter={props.handleRemoveTagFromFilter} tags={props.ignoreTags} color="red"  join="or"/>
        </div>
      ...
      <div className="flex flex-wrap h-full p-5 pt-0 gap-3 overflow-y-scroll" onMouseDown={() => props.handleOpenImage(null)}>
        {imageCards}
      </div>
    </div>
  )
